...
function New-TrackedFont([string]$role, [string]$requested, [float]$size, [System.Drawing.FontStyle]$style = [System.Drawing.FontStyle]::Regular) {
  ...
  $sha256 = (Get-FileHash -LiteralPath $loaded.FontFile -Algorithm SHA256).Hash
  $record = [pscustomobject]@{ Role=$role; Requested=$requested; Resolved=$resolved; Status='PASS'; FontFile=$loaded.RelativePath; Weight=[int]$loaded.Weight; Sha256=$sha256; Size=$size; Style=[string]$style }
...
$log = @(
  ...
  'Font Resolution Policy: PrivateFontCollection exact internal family; Stage B requires manifest SHA, Git tracking and HEAD presence; system fallback forbidden',
  ($script:fontAudit | ForEach-Object { "Font $($_.Role) | Requested Font: $($_.Requested) | Resolved Font: $($_.Resolved) | Font File: $($_.FontFile) | Weight: $($_.Weight) | SHA-256: $($_.Sha256) | Status: $($_.Status)" }),
