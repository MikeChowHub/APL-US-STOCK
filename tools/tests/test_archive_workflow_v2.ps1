...
function New-ProductionFixture([string]$Root) {
  ...
  Write-Text (Join-Path $packageRoot "APL_DeepScan_Social_Radar_Top30_${ScanDate}_1080x1350.png") 'social-radar'
  ...
  $editorialArtifacts=@(
    ...
    @('blog-html-source',(Join-Path $packageRoot "APL_Momentum_Leaders_Market_Analysis_Blog_$ScanDate.html.txt")),
  ...
  $mandatorySections=@([string]$headingMap.ExecutiveSummary,[string]$headingMap.MarketContext,[string]$headingMap.WhyAPL,[string]$headingMap.DeepScanOverview,[string]$headingMap.TopGainers,[string]$headingMap.MomentumLeaders,[string]$headingMap.SectorAnalysis,[string]$headingMap.Risk,[string]$headingMap.DeepScanConclusion)
  ...
  $requiredDefinitions = @(
    ...
    @('social-radar-png',"APL_DeepScan_Social_Radar_Top30_${ScanDate}_1080x1350.png"),
