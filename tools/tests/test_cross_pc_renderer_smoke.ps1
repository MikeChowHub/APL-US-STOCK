...
try {
  ...
  $background=Join-Path $TestRoot 'background.png';$bitmap=[Drawing.Bitmap]::new(1280,1350);$graphics=[Drawing.Graphics]::FromImage($bitmap);try{$graphics.Clear([Drawing.Color]::FromArgb(4,15,24));$bitmap.Save($background,[Drawing.Imaging.ImageFormat]::Png)}finally{$graphics.Dispose();$bitmap.Dispose()}
  $briefPath=Join-Path $TestRoot 'brief.json';$brief=[ordered]@{version='APL Cover Brief v1.0';scanDate='2040-01-02';marketConclusion='Cross-PC smoke';capitalFlow=[ordered]@{from='Cash';to='Leaders'};riskBackground='Smoke';leadershipDestination=@('Technology');mainVisualMetaphor='Light corridor';composition=[ordered]@{textSafeArea='Top';storyArea='Bottom';rules=@('No UI')};imageGenerationBrief=[ordered]@{prompt='Smoke background';negativePrompt='Text'};overlay=[ordered]@{kicker='APL DEEP SCAN';titleLines=@('CROSS-PC','ENVIRONMENT AUDIT');subtitle='Repository managed assets';series='APL US STOCK';date='2040-01-02';footer='Environment smoke'}}
  ...
  & powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -File $overlayScript -BriefPath $briefPath -BackgroundPath $background -OutputPath $cover -Variant Cover -LogoPath $logo -Width 600 -Height 750 | Out-Null
  if($LASTEXITCODE-ne 0){throw 'Cover smoke failed.'};Assert-Png $cover 600 750
  & powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -File $overlayScript -BriefPath $briefPath -BackgroundPath $background -OutputPath $seo -Variant SEO -LogoPath $logo -Width 1280 -Height 720 | Out-Null
