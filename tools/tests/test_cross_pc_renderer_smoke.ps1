...
try {
  ...
  $socialRadarScript=Join-Path $ProjectRoot 'tools\render_deep_scan_social_radar_svg.ps1'
  ...
  & powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -File $socialRadarScript -RegressionTest -InputPath $socialContractPath | Out-Null
  if($LASTEXITCODE-ne 0){throw 'Social Radar SVG smoke failed.'}
  $socialRadarSvg=Join-Path $TestRoot 'APL_DeepScan_Social_Radar_Top30_2040-01-02_1080x1350.svg';$socialRadarPng=Join-Path $TestRoot 'social-radar.png'
  $socialRadarText=[IO.File]::ReadAllText($socialRadarSvg,[Text.Encoding]::UTF8)
  foreach($required in @('Universe','Qualified','Leaders','Leader Lock','BUYABILITY','SECTOR DISTRIBUTION')){if(-not$socialRadarText.Contains($required)){throw "Social Radar role regression missing: $required"}}
  $socialRadarResult=& powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -File $convertScript -RegressionTest -InputSvg $socialRadarSvg -OutputPng $socialRadarPng -Width 1080 -Height 1350
  if($LASTEXITCODE-ne 0){throw 'Social Radar PNG smoke failed.'};Assert-Png $socialRadarPng 1080 1350
  foreach($result in @($dashboardResult,$socialResult,$socialRadarResult)){if([int]$result.FontFallbackWarningCount-ne 0-or[int]$result.InvalidGeometryWarningCount-ne 0){throw 'resvg smoke reported fallback or invalid geometry.'}}
  ...
  foreach($log in @([IO.Path]::ChangeExtension($cover,'.overlay-log.txt'),[IO.Path]::ChangeExtension($seo,'.overlay-log.txt'))){$text=[IO.File]::ReadAllText($log,[Text.Encoding]::UTF8);if($text -match 'Status:\s*FAIL'){throw "Font fallback/failure in $log"};$fontLines=@($text-split"`r?`n"|Where-Object{$_-like'Font *'});foreach($fontName in @('fontKicker','fontTitle','fontSubtitle')){if(@($fontLines|Where-Object{$_-like("Font "+$fontName+" |*")-and$_-like'*Status: PASS'}).Count-ne1){throw "Required font audit record missing: $fontName in $log"}}}
  [pscustomobject]@{Status='PASS';Dashboard='1920x1080';Social='1080x1350';SocialRadar='1080x1350';Cover='600x750';SEO='1280x720';FontFallbackWarningCount=0;InvalidGeometryWarningCount=0;Renderer='resvg';SystemFonts=$false;BrowserDependency=$false}
} finally {
