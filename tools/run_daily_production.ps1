...
$managedInputRoot = $null
if ($hasCompleteManagedInputs) {
  if ($RegressionTest) {
    $managedInputDateRoot = Assert-AplNoReparsePath -Path (Split-Path $InputCsv -Parent) -AllowedRoot $regressionRoot -RequireDirectory
    if ((Split-Path $managedInputDateRoot -Leaf) -cne $ScanDate) { throw "Regression managed-input date directory must be named '$ScanDate'." }
    $managedInputRoot = Split-Path $managedInputDateRoot -Parent
  } else {
    $managedInputRoot = Join-Path $ProjectRoot 'work\managed-inputs'
    $managedInputDateRoot = Assert-AplNoReparsePath -Path (Join-Path $managedInputRoot $ScanDate) -AllowedRoot $managedInputRoot -RequireDirectory
  }
}
...
try {
  ...
  if ($hasCompleteManagedInputs) {
    $preflightArgs = @(
      ...
      '-SeoNativeContractPath',$SeoNativeContractPath,
      '-PublishingArtifactsRoot',$PublishingArtifactsRoot,
      '-ManagedInputsRoot',$managedInputRoot
    )
  ...
  if ($hasCompleteManagedInputs) {
    Complete-InternalStep 'VerifyTriggerBEvidence' {
      ...
      $managedRankingValue = [string]$managedMeta.fullRankingCsv
      $managedRankingCandidate = if ([IO.Path]::IsPathRooted($managedRankingValue)) { $managedRankingValue } else { Join-Path (Split-Path $TriggerBMetaPath -Parent) $managedRankingValue }
      $managedRanking = Assert-AplNoReparsePath -Path $managedRankingCandidate -AllowedRoot $managedRoot -RequireFile
