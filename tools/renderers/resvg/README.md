...
The adapter invokes resvg with repository `Assets/Fonts` via `--use-fonts-dir` and `--skip-system-fonts`; it never depends on Windows-installed fonts. The Dashboard mapping is Alibaba Sans HK 55/75 assets as the resvg family `Alibaba Sans HK` at weights 400/600, and Montserrat assets at weights 400/500/600/700. Until a licensed repository mono font is added, the Dashboard `.mono` class uses Montserrat 500/600. Renderer audit returns the requested family policy and managed font files. Any font-fallback or invalid-geometry warning is a production failure and the partial PNG is deleted.
