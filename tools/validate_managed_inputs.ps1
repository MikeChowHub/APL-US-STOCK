...
$csv=@(Import-Csv -LiteralPath $InputCsv -Encoding UTF8);if($csv.Count-eq 0){throw 'InputCsv is empty.'}
...
[pscustomobject]@{Status='MANAGED INPUT PREFLIGHT PASS';ScanDate=$ScanDate;InputRows=$csv.Count;RequiredTableCards=4;PublishingArtifacts=$requiredPublishing.Count;ProductionStarted=$false}
